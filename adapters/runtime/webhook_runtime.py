...
class WebhookRuntime:
    ...
    async def wait_for_response(
        ...
        *,
        ...
        timeout = self.response_timeout_seconds if timeout_seconds is None else float(timeout_seconds)
        try:
            return await asyncio.wait_for(asyncio.shield(future), timeout=timeout)
        except asyncio.TimeoutError as exc:
            self.discard_response(correlation_id)
            raise TimeoutError(f"Webhook response timed out after {timeout:.1f}s.") from exc
        finally:
    ...
    def fail_response(self, correlation_id: str, detail: str) -> bool:
        ...
        if future is None or future.done():
            return False
        future.set_exception(RuntimeError(str(detail).strip() or "Webhook response failed."))
        return True

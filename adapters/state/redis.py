...
from shared_types.models import EMOTIONS, LocationState, MOOD_NEUTRAL, MoodState
...
class RedisStateStore:
    ...
    async def set_mood(self, mood: MoodState) -> None:
        payload = {key: value for key, value in mood.clamp().as_dict().items()}
    ...
    async def init_defaults(self) -> None:
        if not await self._client.exists(MOOD_KEY):
            await self._client.hset(MOOD_KEY, mapping={key: MOOD_NEUTRAL for key in EMOTIONS})
        if not await self._client.exists(LIVE_LOCATION_KEY):
            await self._client.hset(LIVE_LOCATION_KEY, mapping=LocationState().as_dict())
        if not await self._client.exists(PINNED_LOCATION_KEY):
            await self._client.hset(PINNED_LOCATION_KEY, mapping=LocationState().as_dict())

...
class ReminderTool(BaseTool):
    ...
    _output: OutputAdapter = PrivateAttr()
    ...
    async def _arun(
        ...
        if envelope.source == MessageSource.REMINDER:
            return "Reminder could not be scheduled from a reminder delivery."
...
def _envelope_from_state(state: dict[str, Any] | None) -> MessageEnvelope:
    ...
    return MessageEnvelope(source=MessageSource.INTERNAL, content="")
...
def _resolve_route(
    *,
    ...
    target_user_id = str(envelope.author_id or fallback_user_id or "").strip() or None
    if target_user_id:
        return {"target_user_id": target_user_id, "channel_id": None}

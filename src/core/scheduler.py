...
from shared_types.types import ScheduledTask, ScheduleRequest
    ...
    async def tick(self) -> int:
        ...
        for task in tasks:
            ...
            try:
                ...
                print(
                    f"[task_scheduler] publish failed (task_id={task.get('id')}): {exc}"
                )
        return sent


def _build_task(
    request: ScheduleRequest, *, fallback_target_user_id: str | None
) -> ScheduledTask:
...
def _resolve_due_ts(request: ScheduleRequest, *, mode: str) -> float:
    if mode == "window":
        ...
        if end_ts < start_ts:
            raise ValueError("ScheduleRequest.window_end must be >= window_start.")

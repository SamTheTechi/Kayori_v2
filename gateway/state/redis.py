...
from shared_types.models import InteractionState, LifeNote, MessagesHistory, MoodState, Todo
...
TODOS_KEY_PREFIX = "kayori:state:todos"
...
class RedisStateStore:
    ...
    async def get_todos(self) -> list[Todo]:
        raw = await self._client.get(_key(TODOS_KEY_PREFIX))
        if not raw:
            return []
        try:
            payload = _json_value(raw)
        ...
        if not isinstance(payload, list):
            return []
        return [Todo.from_dict(item) if isinstance(item, dict) else Todo() for item in payload]

    async def add_todo(self, todo: Todo) -> None:
        todos = await self.get_todos()
        todos.append(todo)
        await self._client.set(_key(TODOS_KEY_PREFIX), _json([t.to_dict() for t in todos]))

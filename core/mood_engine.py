...
class MoodEngine:
    ...
    def apply(self, current: MoodState, delta: dict[str, float]) -> MoodState:
        ...
        for tone in EMOTIONS:
            ...
            change = float(delta.get(tone, 0.0)) * \
            ...
            for conflict in conflicts.get(tone, []):
                ...
                value -= conflict_distance * \
                    self.conflict_multiplier

            for reinforce in reinforces.get(tone, []):
                reinforce_distance = float(
                    getattr(next_state, reinforce)) - MOOD_NEUTRAL
                value += reinforce_distance * \
    ...
    def spike(self, current: MoodState, low: float = 0.08, high: float = 0.2) -> MoodState:
        ...
        spike_scale = (
            self.slow_spike_scale
            if tone in SLOW_EMOTIONS
            else self.fast_spike_scale
        )
        value = float(getattr(next_state, tone))
    ...
    def reaction_from_delta(self, delta: dict[str, float], threshold: float = 0.75) -> str:
